...
MessageStorage::MessageStorage(const DbConfig& db_cfg)
    : db_cfg_(db_cfg),
      ready_(false),
      db_enabled_(false),
      logger_(Poco::Logger::get("signaling_storage"))
{
    if (!db_cfg_.valid()) {
    ...
    try {
        initializeDatabase();
    } catch (const BrokenStorageError& e) {
        throw;
    } catch (const Poco::Exception& e) {
        logger_.error(Poco::format("Failed to initialize database connection: %s", e.displayText()));
        throw BrokenStorageError(Poco::format("cannot init storage. reason: %s", e.displayText()));
    }
}
...
void MessageStorage::initializeDatabase()
{
...
void MessageStorage::testConnection()
{
    try {
...
int MessageStorage::createEvent(const EventData& event_data)
{
    try {
        std::lock_guard<std::mutex> lock(pool_mutex_);
...
std::vector<std::string> MessageStorage::getUniqueValues(const std::string& field_name)
{
...
std::vector<EventDataExtended> MessageStorage::findEvents(double timestamp,
...
std::vector<EventDataExtended> MessageStorage::findEventsByType(const std::string& event_type)
{
    try {
        std::lock_guard<std::mutex> lock(pool_mutex_);
        Poco::Data::Session session(pool_->get());
...
std::vector<EventDataExtended> MessageStorage::resultToEventDataExtendedList(
    const std::vector<std::vector<Poco::Dynamic::Var>>& data_list)
{
    ...
    for (const auto& row : data_list) {
        ...
        try {
            ...
            // Handle tags
            std::string tags_str = row[6].convert<std::string>();
            // Tags are stored as text array in PostgreSQL, but we're not using them directly
